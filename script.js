...
// ── AUTO-LOGIN / SESSION MANAGEMENT (fixed) ───────────────
function saveSession(user) {
  if (user) {
    const toStore = {
      ...
      email: user.email,
      timestamp: Date.now()  // optional: track when saved
    };
    localStorage.setItem('aimlab_user', JSON.stringify(toStore));
...
async function tryAutoLogin() {
  const savedRaw = localStorage.getItem('aimlab_user');
  if (!savedRaw) {
  ...
  try {
    ...
    if (!saved.access_token) {
      console.log('❌ Saved session missing access_token');
      return false;
    }
    console.log('🔄 Attempting auto-login with token for user:', saved.email);
    
    // Verify token by fetching profile
    const prof = await supabase(`/rest/v1/profiles?id=eq.${saved.id}&select=*`, {}, saved.access_token);
    if (!prof.ok || !prof.data?.length) {
      console.log('❌ Auto-login failed: token invalid or profile not found');
      localStorage.removeItem('aimlab_user');
      return false;
    }
    currentUser = { id: saved.id, access_token: saved.access_token, email: saved.email };
    currentProfile = prof.data[0];
    console.log('✅ Auto-login successful! Welcome back', currentProfile.username);
    await enterMenu();
    return true;
  } catch(e) {
