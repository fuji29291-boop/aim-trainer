...
let trackingMonitorInterval = null;  // for tracking mode target count
...
function startGame() {
  ...
  if (selectedMode === 'tracking') {
    startTracking();
    // Monitor to keep at least 2 tracking targets alive
...
// ── REACTION MODE (fixed early click) ────────────────────
...
// Early click handler (called from missClick)
function handleEarlyReactionClick() {
  ...
  reactionRound++;  // advance round without recording time
...
// ── SPAWN TARGET (tracking now has despawn timer) ────────
function spawnTarget() {
  if (!isGameRunning || selectedMode === 'reaction' || selectedMode === 'trace') return;
  if (selectedMode === 'tracking' &&
      document.querySelectorAll('.target.tracking').length >= 6) return; // soft cap
  ...
  if (selectedMode==='tracking') {
    ...
    // Despawn tracking targets after 8 seconds
...
// ── HIT (fixed popup position, tracking respawn) ─────────
function hitTarget(el) {
  ...
  // Get accurate current position for popup
  ...
  // For tracking mode: spawn a new target immediately to keep count up
  if (selectedMode === 'tracking' && isGameRunning) {
    setTimeout(() => spawnTarget(), 50);
  }
}

// ── MISS (handles reaction early click) ─────────────────
function missClick(e) {
  ...
  if (selectedMode === 'reaction') {
    ...
    // If in 'go' state and misses background, ignore (target handles hit)
    return;
  }
...
// ══════════════════════════════════════════════════════════
//  CROSSHAIR SYSTEM (with persistence)
// ══════════════════════════════════════════════════════════
let _xhairEl = null;
...
window.addEventListener('DOMContentLoaded', () => {
  // Hide system cursor everywhere, but restore on touch devices (handled in CSS)
  const styleTag = document.createElement('style');
  styleTag.textContent = 'html,body,*{ cursor:none !important; }';
  document.head.appendChild(styleTag);

  initCrosshair();
  // Apply saved difficulty to UI
  setDifficulty(settings.difficulty);
  // Highlight correct crosshair button
