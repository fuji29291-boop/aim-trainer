...
function startTracking() {
  ...
  function moveTargets() {
    ...
    document.querySelectorAll('.target.tracking').forEach(t => {
      ...
      const r   = 30; // fixed radius
      // Random gentle curve — drift angle slightly each frame
...
function hitTarget(el, x, y) {
  ...
  // Mode multipliers — each mode has different difficulty
  if (selectedMode === 'flick')    points = Math.round(points * 2.0); // hardest: 600ms window
  if (selectedMode === 'tracking') points = Math.round(points * 1.6); // moving targets
  if (selectedMode === 'switching' && el.classList.contains('priority')) { points += 180; bonusText = '+180 PRIORITY!'; }
  // Streak multiplier (every 5 hits +10% up to +50%)
  ...
  // Score popup at actual element position for tracking mode
  const elRect = el.getBoundingClientRect();
  const gaRect = document.getElementById('game-area').getBoundingClientRect();
  const popX = elRect.left - gaRect.left + elRect.width/2;
  const popY = elRect.top  - gaRect.top  + elRect.height/2;
  ...
  showHitRing(popX, popY); el.remove();
}
...
async function loadLeaderboard() {
  ...
  try {
    const { ok, data } = await supabase(
      `/rest/v1/scores?mode=eq.${currentLbTab}&select=name,score,accuracy,hits&order=score.desc&limit=500`
    );
    if (!ok || !data.length) {
      list.innerHTML = '<div class="lb-loading">No scores yet for this mode — be the first!</div>';
      return;
    }
    // Deduplicate: keep only each player's best score (sorted desc, so first = best)
    const seen = new Map();
    for (const row of data) { if (!seen.has(row.name)) seen.set(row.name, row); }
    const unique = Array.from(seen.values()).slice(0, 50);
    ...
    if (myScore > 0) {
      const myPos = unique.findIndex(r => r.name === myName);
      if (myPos !== -1) { yourRankNum.textContent = '#'+(myPos+1); yourRankEl.style.display = 'block'; }
      else { const myRank = await fetchModeRank(currentLbTab, myScore); if (myRank) { yourRankNum.textContent = '#'+myRank; yourRankEl.style.display = 'block'; } }
    }
  } catch(e) {
